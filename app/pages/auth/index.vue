<template>
    <Filter v-model:filterActive="filterActive" v-model:filterSearch="filterSearch" creatable/>
...
<script setup>
    ...
    watchEffect(async () => {
        filteredGroups.value = await $fetch(`/api/db/ex_department/${userInfo.value.department_id}?filter=${filterActive.value}`,{
            method: 'GET'
        })
    })
</script>
